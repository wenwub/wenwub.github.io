...
window.onload = function () {
  ...
  if (G_VAR.sTest != undefined) {
    ...
    G_VAR.getDictList =
      'http://127.0.0.1/door/base/public/web/getDictList?sFiledEName=sMenuTypeCode' // 获取H5菜单的地址
...
var G_VAR = {
  ...
  getDictList:
    SysUtils.getHttpRoot() +
    '/base/public/web/getDictList?sFiledEName=sMenuTypeCode', // 获取H5菜单的地址
  ...
  showNotice() {
    ...
    setInterval(() => {
      if (G_VAR.iNoticeIndex == 0) {
        ...
        if (G_VAR.arrNoticeList.length == 2) {
          // 2条数据
    ...
    if (G_VAR.arrMenuList.length != 0) {
      ...
      $.each(G_VAR.arrMenuList, (index, item) => {
        if (item.items.length != 0) {
          str +=
            '<div class="main_box">' +
            '<div class="main_title"><img src="img/title_left.png">' +
            item.title +
            '<img src="img/title_right.png"></div>' +
