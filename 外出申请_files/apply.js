...
window.onload = function () {
  ...
  if (Global_VAR.sTest != undefined) {
    Global_VAR.initData = 'http://127.0.0.1/out/h5/get' // 初始化数据
  }
...
var Global_VAR = {
  ...
  initData: SysUtils.getHttpRoot() + '/get', // 查询接口更改，自己建了一个地方
...
var Global_FN = {
  ...
  getRand() {
    Global_VAR.rand = Math.floor(Math.random() * (10 - 0) + 1)
  },
  ...
  getInitData() {
    layui.use(['form', 'jquery'], function () {
      ...
      $.ajax({
        url: Global_VAR.initData + Global_VAR.rand,
        type: 'GET', //请求方式为get
        ...
        success: function (res) {
          if (res['code'] == 1) {
            ...
            d.dOutStartDate != null ? $('.dOutStartDate').text(rDate) : ''
            d.dOutEndDate != null ? $('.dOutEndDate').text(rDate) : ''
      ...
      $('#' + value).on('click', function () {
        weui.datePicker({
          ...
          onConfirm: function (result) {
            ...
            var mm = '',
              dd = ''
            result[1].value.toString().length === 1
              ? (mm = '0' + result[1].value)
              : (mm = result[1].value)
            result[2].value.toString().length === 1
              ? (dd = '0' + result[2].value)
              : (dd = result[2].value)
    ...
    layui.use(['jquery'], function () {
      ...
      weui.picker(Global_VAR.hoursList, {
        ...
        onConfirm: function (result) {
          ...
          $('#dSiteDate').text(
            Global_VAR.dSiteDate + ' ' + Global_VAR.sSiteTime
          )
        },
        id: 'singleLinePicker',
      })
